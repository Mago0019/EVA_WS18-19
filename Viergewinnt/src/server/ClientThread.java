...
public class ClientThread extends Thread
{
	...
	{ // ich bin Spieler 2 und joine einem Spiel
	...
	private synchronized void leaveGameSession()
	{
		...
		{ // Falls der Client schon geschlossen ist
	...
	{ // Du bist Host und hast das Spiel gestartet.
	...
	private void win_lose(boolean win)
	{ // ich habe aufgegeben -> win = false
		try
		{
			if (iAmHost)
			{ // bin Host
	...
	private void setStone(int collumn)
	{
		...
		if (correct)
		{
			...
			if (this.gameSession.gameField.checkWin(collumn, playerNr))
			{ // Überprüfen, ob ich gewonnen hab
				win_lose(true);
			...
			} else
			{
				this.output.println("~~31false" + ";" + gameSession.gameFieldToString());
				if (iAmHost)
				{ // anderem Clienten bescheid geben, wer am Zug ist
					this.gameSession.player2.output.println("~~31true" + ";" + gameSession.gameFieldToString());
				} else
				{
					this.gameSession.player1.output.println("~~31true" + ";" + gameSession.gameFieldToString());
	...
	private void logoutClient()
	{
		...
		{ // bin min in der Lobby
			if (this.gameSession.playerTurn != 0)
	...
	private void askForPlayerName() throws IOException
	{
		...
		if (trycount > 3)
		{ // Falls sich der Client nicht mehr gemeldet hat
